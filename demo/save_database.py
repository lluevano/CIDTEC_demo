...
for subject_id,folder in enumerate(database_folders):
    ...
    im_tensor = np.zeros((len(images), 112, 112, 3))
    #for img_idx in range(len(images)):

    for i,image in enumerate(images):
        _, lmks = fr_pipeline.detectFaces(image)
 ...
 #   model.set_params(arg_params,aux_params)
    ...
    if len(images)>1:
        ...
        sum_emb /= np.linalg.norm(sum_emb,ord=2,axis=0)
    else:
        sum_emb = embeddings
    #model._reset_bind()
    #print(sum_emb)
...
with open(filename, 'wb') as database_file:
  # Step 3
  pickle.dump({'biometric_templates': np.array(identity_vectors), 'labels': np.array(labels)}, database_file)
